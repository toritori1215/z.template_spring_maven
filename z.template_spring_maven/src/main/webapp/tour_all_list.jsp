	...
	$('input:checkbox').click(function(e) {
		...
		$.ajax({
			...
			success:function(jsonObject) {
				var tourJsonArray = jsonObject.data;
				var html = "";
				/*
				html="<div id=\"productList\" class=\"strip_all_tour_list wow fadeIn\" data-wow-delay=\"0.1s\"> "+
						 "<div class=\"row\"> <div class=\"col-lg-4 col-md-4\"> <div class=\"wishlist\"> "+
						 "<a class=\"tooltip_flip tooltip-effect-1\" href=\"javascript:void(0);\">+<span class=\"tooltip-content-flip\"><span class=\"tooltip-back\">Add to wishlist</span></span></a></div> "+
						 "<div class=\"img_list\"> <a href=\"tour_detail?pNo=${product.pNo}\"><img src=\"${pageContext.request.contextPath}/resources/z.SiliconVillage/img/${product.pName}1.jpg\" alt=\"Image\"> "+
						 "";
				*/
				for (var i = 0; i < tourJsonArray.length; i++) {
					var tourJsonObject = tourJsonArray[i];
					var pNo = tourJsonObject.pNo;
					var pName = tourJsonObject.pName;
					var foodCategory = tourJsonObject.foodCategory;
					var pRate = tourJsonObject.pRate;
					var rCount = tourJsonObject.rCount;
					var pDesc = tourJsonObject.pDesc;
					var pPrice = tourJsonObject.pPrice;
					...
					html += 
						...
						"<div class='col-lg-4 col-md-4'>"+
							...
							"</div>"+
							"<div class='img_list'>"+
								"<a href='tour_detail?pNo="+pNo+"'><img src='${pageContext.request.contextPath}/resources/z.SiliconVillage/img/"+pName+"1.jpg' alt='Image'>"+
									"<div class='short_info'>"+
									"<c:choose>"+
										"<c:when test=\""+foodCategory+" eq 'water'\">"+
									       	"<i class='pe-7s-drop'></i>WATER"+
									    "</c:when>"+
									    "<c:when test='"+foodCategory+' == "ice"'+"'>"+
						...
						"<div class='col-lg-6 col-md-6'>"+
							"<div class='tour_list_desc'>"+
								"<div class='rating'>"+
								"<c:forEach var='index' begin='1' end='"+Math.round(pRate/2)+"'>"+
								"<i class='icon-smile voted'></i>"+
								"</c:forEach>"+
								"<c:forEach var='index' begin='1' end='"+(5-Math.round(pRate/2))+"'>"+
								"<i class='icon-smile'></i>"+
								"</c:forEach>"+
								"<small>("+rCount+")</small>"+
								"</div>"+
								"<h3><strong>"+pName+"</strong> tour</h3>"+
								"<p>"+pDesc+"</p>"+
								"<ul class='add_info'>"+
									"<li>"+
										"<div class='tooltip_styled tooltip-effect-4'>"+
						...
						"<div class='col-lg-2 col-md-2'>"+
							"<div class='price_list'>"+
								"<div><sup>￦</sup>${product.pPrice/10000}*<small><br><b>Per person</b></small><small><br>*가격 단위: 1만원</small>"+
									"<p><a href='tour_detail?pNo="+pNo+"' class='btn_1'>Details</a>"+
									"</p>"+
								"</div>"+
							"</div>"+
						"</div>"+
